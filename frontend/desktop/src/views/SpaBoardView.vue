...
function onPanStart(e) {
  ...
  if (
    e.target.closest(
      ".card, .board-drag, .board-head h3, input, textarea, button, a",
    )
  )
    return;
  isPanning = true;
  ...
  <section v-if="spa">
    ...
    <div class="board-row" ref="boardRow" @pointerdown="onPanStart">
      <div class="boards" ref="boardsContainer">
        <div
          v-for="board in spa.boards"
          :key="board.id"
          class="board board-col"
        ...
        >
          <header class="board-head">
            ...
            <template v-else>
              <h3 @click="startBoardEdit(board)" title="點擊改名">
                {{ board.name }}
              </h3>
          ...
          <div class="sort-pills" role="group" aria-label="排序方式">
            ...
            >
              <span v-if="board.sort_mode === 'unicode'" class="check">✓</span
              >預設(標題)
            </button>
            <button
              class="pill"
            ...
            >
              <span v-if="board.sort_mode === 'manual'" class="check">✓</span
              >手動
            </button>
          </div>
          ...
          <ul
            class="card-list"
            :data-board-id="board.id"
            :data-sort-mode="board.sort_mode"
          >
            <li
